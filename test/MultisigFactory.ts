...
describe("MultisigFactory", function () {
  ...
  async function deployContractAndSetVariable() {
    ...
    const quorum = 4;
    const MultisigFactory = await hre.ethers.getContractFactory("MultisiFactory");
    const multisigfactor= await MultisigFactory.deploy(quorum, signers);
    const erc20Token = await hre.ethers.getContractFactory("TobiXI");
    const token = await erc20Token.deploy();
    const len = signers.length;
    ///declare variables here
    return { multisigfactor, owner,len, account1,account2,account3, account4,account5, quorum, token, recipient};
  }

  describe("Deployment testing", function(){
    it("Should check if valid signer is greater than one", async function () {
      const {multisigfactor, len} = await loadFixture(deployContractAndSetVariable);
      await expect(len).to.be.gt(1);
    });
  
    it("Should check if qorum is greater than one", async function () {
      const {multisigfactor, quorum} = await loadFixture(deployContractAndSetVariable);
      await expect(quorum).to.be.gt(1);
    });

    it("Should check if address zero is among valid signers", async function () {
      const {multisigfactor, quorum} = await loadFixture(deployContractAndSetVariable);
      await expect(multisigfactor).to.not.revertedWith("zero address not allowed");
    });

    it("Should check if signers already exist", async function () {
      const {multisigfactor, quorum} = await loadFixture(deployContractAndSetVariable);
      await expect(multisigfactor)
      .to.not.revertedWith("signer already exist");
    });
    it("Should check if quorum is greater than valid signers", async function () {
      const {multisigfactor, quorum} = await loadFixture(deployContractAndSetVariable);
      await expect(multisigfactor).to.not.revertedWith("quorum is greater than valid signers");
    });  
  });
